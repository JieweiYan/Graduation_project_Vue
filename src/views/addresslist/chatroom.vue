<template>
  <div>
    <el-card  shadow="never" v-for="(item, index) in massagelist" style="padding: 0px 10px 0px 10px">
      <el-row style="align-content: center; margin: 0px 10px 0px 10px">
      ...
      <el-row style="align-content: center; margin: 1px">
        <el-col style="text-align: right">
          <p style="font-size: 12px; color: #838589">{{ totlepost - (index + (currentPage - 1)*12)  }}楼 {{item.date}}</p>
        </el-col>
      </el-row>
    </el-card>
...
export default {
  ...
  created() {
    ...
    var this1 = this
    axios.get('http://localhost:8100/chatroommassage/getmassagenum/'+id).then(function (resp){
      this1.totlepost = resp.data
    })
    axios.get('http://localhost:8100/chatroommassage/getmassage/'+id+'/'+this.currentPage).then(function (resp){
      console.log(resp)
      console.log("hhh")
      this1.massagelist = resp.data.records
  ...
  methods: {
    gotousercenter(index) {
      this.$router.push({path:'/usercenter', query:{id:this.massagelist[index].userid}})
    },
    //想获取文本编译框内的html，可以添加事件获取
    submit(){
      var xss = require("xss");
      const safeHtml = xss(editor.txt.html())
      this.massage.content = safeHtml;
      var this1 = this
      var id = window.localStorage.getItem("id")
      var token = window.localStorage.getItem("token")
      axios.post('http://localhost:8100/chatroommassage/reply/'+id+'/'+token, this.massage).then(function (resp) {
        if(resp.data == ""){
